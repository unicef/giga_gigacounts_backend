...
test.group('Create Payment', (group) => {
  ...
  test('Successfully create a payment in August 2022', async ({ client, expect }) => {
    ...
    const body = await buildBody(8, 2022, contract.id.toString(), contract.currencyId.toString())
  ...
  test('Successfully create a payment in June 2022', async ({ client, expect }) => {
    const user = await setupUser('Government')
    const contract = await setupModels(user.countryId, user.id)
    const body = await buildBody(7, 2022, contract.id.toString(), contract.currencyId.toString())
    const response = await client.post('/payment').loginAs(user).json(body)
    const payment = response.body() as Payment
...
const buildBody = async (
  ...
  amount: 100000,
  invoice: {
    file: await testUtils.toBase64('data:application/pdf;base64,'),
    name: 'File',
  },
  contractId,
  currencyId,
