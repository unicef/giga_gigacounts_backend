...
test.group('List ISPs', (group) => {
    ...
    const user = await UserFactory.with('country', 1)
      .with('roles', 1, (role) =>
        role
          ...
          .with('permissions', 1, (permission) => permission.merge({ name: 'isp.read' }))
      )
      .create()
  ...
  test('Succesfully return all ISPs by Country if user is gov', async ({ client, expect }) => {
    const country = await CountryFactory.create()
    const country2 = await CountryFactory.merge({ name: 'countryTwo' }).create()
    const isp1 = await IspFactory.merge({ countryId: country.id, name: 'T-Mobile' }).create()
    const isp2 = await IspFactory.merge({ countryId: country.id, name: 'Verizon' }).create()
    ...
    const isps = response.body() as Isp[]
    expect(isps.length).toBe(1)
    expect(isps.some((i) => i.name === isp1.name)).toBeFalsy()
    expect(isps.some((i) => i.name === isp2.name)).toBeFalsy()
    expect(isps.some((i) => i.name === isp3.name)).toBeTruthy()
  })
  test('Succesfully return all ISPs by Country if user is country office', async ({
    client,
    expect,
  }) => {
    const country = await CountryFactory.create()
  ...
  test('Succesfully return all ISPs by LTA by Country if user is country office', async ({
    client,
    expect,
  }) => {
    const country = await CountryFactory.create()
  ...
  test('Succesfully return empty ISPs by LTA if the user is not admin and the country is different', async ({
    client,
    expect,
  }) => {
    const country = await CountryFactory.create()
    ...
    const response = await client.get(`/isp?ltaId=${att.ltas[0].id}`).loginAs(user)
    const isps = response.body() as Isp[]
    expect(isps.length).toBe(0)
    expect(isps.some((i) => i.name === 'T-Mobile')).toBeFalsy()
    expect(isps.some((i) => i.name === 'Verizon')).toBeFalsy()
    expect(isps.some((i) => i.name === 'AT&T')).toBeFalsy()
  })
})
