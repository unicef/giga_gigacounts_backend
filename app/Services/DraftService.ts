  ...
  try {
    const draft = await Draft.create({
      ...
      launchDate: draftData.launchDate
        ? utils.formatContractDate(draftData?.launchDate, true)
        : undefined,
...
const duplicateDraft = async (draftId, user: User) => {
  ...
  try {
    ...
    const newDraft = await Draft.create({
      ...
      paymentReceiverId: draft.paymentReceiverId
    })

    const ispContacts = await draft.related('ispContacts').query().exec()
    for (const ispContact of ispContacts) {
      await newDraft.related('ispContacts').attach([ispContact.id])
    }

    const externalContacts = await draft.related('externalContacts').query().exec()
    for (const externalContact of externalContacts) {
      await newDraft.related('externalContacts').attach([externalContact.id])
    }

    const stakeholders = await draft.related('stakeholders').query().exec()
    for (const stakeholder of stakeholders) {
      await newDraft.related('stakeholders').attach([stakeholder.id])
    }
