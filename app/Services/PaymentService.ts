...
import attachmentService, { AttachmentsType } from 'App/Services/AttachmentService'
...
import contractDTO, { ConnectionMedian } from 'App/DTOs/Contract'
...
const getPaymentsByContract = async (
  contractId: string,
  startDate: DateTime,
  endDate: DateTime
) => {
  ...
  const payments = await Payment.query()
    ...
    .andWhereBetween('createdAt', [startDate.toSQLDate()!, endDate.toSQLDate()!])
    .preload('currency')
  ...
  await Promise.all(
    payments.map(async (payment) => {
      ...
      if (payment.metrics) {
        const connectionMedianBetweenDates = await Database.rawQuery(
          `SELECT contract_id, metric_id, Metrics.name as metric_name, Metrics.unit as unit,
          ROUND(AVG(value)) as median_value
          FROM Measures
          INNER JOIN Metrics ON Metrics.id=metric_id
          WHERE contract_id = ?
          AND Measures.created_at >= ?
          AND Measures.created_at <= ?
          GROUP BY contract_id, metric_id, metric_name, unit`,
          [
            payment.contractId,
            utils.setDateToBeginOfDayFromISO(payment.dateFrom).toSQL()!,
            utils.setDateToEndOfDayFromISO(payment.dateFrom).toSQL()!
          ]
        )

        payment.metrics = {
          connectionsMedian: (await connectionMedianBetweenDates.rows) as ConnectionMedian[],
          withoutConnection: 0,
...
const getConnectionsMedian = async (contract: Contract, dateFrom: Date, dateTo: Date) => {
  let connectionsMedian: ConnectionMedian[] = []
  dateTo.setHours(23, 59, 59, 999)
  const formattedDateFrom = dateFrom.toISOString()
  const formattedDateTo = dateTo.toISOString()

  if (contract.schools?.length) {
    const schoolIds: number[] = contract.schools.map((school) => school.id)
    ...
    connectionsMedian = (await result.rows) as ConnectionMedian[]
  }

  return connectionsMedian
}
...
const getPercentages = async (
  dateFrom: Date,
  dateTo: Date,
  connection: SchoolConnectionInfo[],
  connectionsMedian?: any[]
) => {
  ...
  const connectionInfo: ConnectionInfo = {
    ...
    connectionsMedian: connectionsMedian
  }
...
const queryBuilder = async (
  ...
  query.whereRaw('date_from::DATE >= ? AND date_to::DATE <= ?', [
    startDate.toSQLDate()!,
    endDate.toSQLDate()!
  ])
  return { query }
...
const getPayments = async (
  ...
  const { query } = await queryBuilder(user, startDate, endDate, countryId)
...
const getPayment = async (paymentId: string) => {
  ...
  if (payment.metrics) {
    ...
    const connectionMedianBetweenDates = await Database.rawQuery(sqlQuery, [
      payment.contractId,
      payment.dateFrom.toSQL()!,
      payment.dateTo?.toSQL()!
    ])
...
const getPaymentMeasures = async (data: PaymentMeasures) => {
  ...
  const connectionValues = await getSchoolMeasures(
    contract[0],
    new Date(dateFrom),
    new Date(dateTo)
  )

  const connectionMedian = await getConnectionsMedian(
    contract[0],
    new Date(dateFrom),
    new Date(dateTo)
  )
